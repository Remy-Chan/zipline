...
class IBBroker(Broker):
    ...
    def positions(self):
        ...
        for symbol in self._tws.positions:
            ...
            # (is this used to calculate gains/losses?, if so should it come from market data?)
            z_position.last_sale_date = None  # TODO(tibor): Fill from state
            z_positions[symbol_lookup(symbol)] = z_position

        return z_positions

    # I feel like the Portfolio is over a set period of time like 1 day, while Account does
    # not depend on time.
    @property
    def portfolio(self):
        ib_account = self._tws.accounts[self.account_id][self.currency]

        z_portfolio = zp.Portfolio()   # good ref: zipline.finance.performance.period Lines 18-68
        z_portfolio.capital_used = None  # TODO(tibor) the dollar amount of securities bought or sold, can be negative
        ...
        z_portfolio.start_date = None  # TODO(tibor): get from state
        z_portfolio.positions = self.positions
        z_portfolio.positions_value = None  # TODO(tibor)
        z_portfolio.positions_exposure = None  # TODO(tibor) net exposure
        # sum of longs dollar value + sum of of shorts dollar value
        # in Q performance:
        # - leverage = account.leverage (1.54 for my l/s strategy)
        # - exposure = account.net_leverage  (0.14 for my l/s stragey)
    ...
    def account(self):
        ...
        z_account.total_positions_value = None  # TODO(tibor) same as total_positions_exposure?
        z_account.total_positions_exposure = None  # TODO(tibor) sum of longs - sum of shorts
        ...
        z_account.available_funds = None  # TODO(tibor), cash?
        ...
        z_account.net_leverage = None  # TODO(tibor) sum of longs + sum of shorts
